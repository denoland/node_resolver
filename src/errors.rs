...
pub fn err_invalid_package_config(
  ...
  Error::msg(msg) // generic_error(msg)
}

pub fn err_module_not_found(path: &Path, base: &Path, typ: &str) -> Error {
  // generic_error
  Error::msg(format!(
...
pub fn err_unsupported_dir_import(path: &str, base: &str) -> Error {
  // generic_error
  Error::msg(format!("[ERR_UNSUPPORTED_DIR_IMPORT] Directory import '{}' is not supported resolving ES modules imported from {}", path, base))
}

/*
pub fn err_unsupported_esm_url_scheme(url: &Url) -> Error {
  let mut msg =
